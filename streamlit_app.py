...
# Estilos CSS - Blindaje de Interfaz y Botones
st.markdown("""
    <style>
    .main { background-color: #f8f9fa; }
    [data-testid="stMetric"] { background-color: #ffffff; border-radius: 10px; padding: 15px; box-shadow: 2px 2px 8px rgba(0,0,0,0.05); }
    .stButton>button { width: 100%; border-radius: 5px; height: 3.5em; font-weight: bold; background-color: #ffffff; border: 1px solid #dcdcdc; }
    .stButton>button:hover { border: 1px solid #e67e22; color: #e67e22; }
...
def load_data():
    ...
    # Limpieza de nulos Crítica para evitar errores de graficación
    for col in ['Objetivo_Mensual', 'Total_Acumulado', 'Promedio', 'Comp_Porc']:
...
def get_ant(fecha):
    ...
    if a > 0: return f"{a} años y {m} meses"
    return f"{m} meses"
...
try:
    ...
    # =========================================================
    # DIMENSIÓN 1: PERFORMANCE COMERCIAL (RESTAURADA Y BLINDADA)
    # =========================================================
    if dimension == "Performance Comercial":
        ...
        with col_det:
            ...
            # Evolución Mensual - BLINDADA
            fig_i = go.Figure()
            fig_i.add_trace(go.Bar(x=lista_meses, y=[v_data[m] for m in lista_meses], name="Ventas", marker_color='#3498db', text=[v_data[m] for m in lista_meses], textposition='auto'))
        ...
        # Gráficos Pie y Box - RESTAURADOS
        st.divider()
        g1, g2 = st.columns(2)
        with g1: 
        ...
        with g2: 
            ...
            st.markdown("""<div class="analisis-box"><b>🧠 Análisis Virtual:</b> Las cajas muestran el equilibrio del equipo. Los puntos aislados (outliers) son tus superestrellas.</div>""", unsafe_allow_html=True)

    # =========================================================
    # DIMENSIÓN 2: MATRIZ 9-BOX (CORREGIDA Y VISUAL)
    # =========================================================
    elif dimension == "Matriz 9-Box Comercial":
        st.markdown("### 🎯 Matriz de Talento 9-Box")
        
        # Filtros Superiores Matriz
        m_f1, m_f2, m_f3 = st.columns(3)
        with m_f1: sel_mes = st.selectbox("Seleccionar Mes para Resultados", lista_meses)
        with m_f2: 
            op_e9 = [x for x in sorted(df_raw['Empresa'].dropna().unique()) if str(x).upper() != "EMPRESA"]
            f_emp9 = st.selectbox("Empresa ", ["Todas"] + op_e9)
        with m_f3: 
            op_l9 = [x for x in sorted(df_raw['Localidad'].dropna().unique()) if str(x).upper() != "LOCALIDAD"]
        ...
        # Eje X: Normalización basada en el mes seleccionado
        ...
        cats = ["Dilema", "E. Emergente", "ESTRELLA", "Cuestionable", "Core Player", "High Performer", "Bajo Rend.", "En Riesgo", "Eficaz"]
        cols_b = st.columns(3)
        cols_b2 = st.columns(3)
        cols_b3 = st.columns(3)
        
        all_cols = cols_b + cols_b2 + cols_b3
        cat_sel = None
        for i, b_col in enumerate(all_cols):
            if b_col.button(cats[i]):
                cat_sel = cats[i]

        # GRÁFICO 9-BOX - BLINDADO CONTRA [NAN]
        # Forzamos que 'size' no tenga valores nulos o negativos
        df_9['Size_Marker'] = df_9['Total_Acumulado'].clip(lower=0.1).fillna(0.1)

        fig_9 = px.scatter(
            df_9, x='Res_Porc', y='Comp_Porc', text='Iniciales', color='Empresa',
            size='Size_Marker', hover_name='Vendedor',
            range_x=[-5, 110], range_y=[-5, 110],
            labels={'Res_Porc': f'Resultados {sel_mes} (%)', 'Comp_Porc': 'Competencias (%)'},
            height=700, template="plotly_white"
        )
        fig_9.update_traces(textposition='middle center', marker=dict(opacity=0.7, line=dict(width=1, color='DarkSlateGrey')))
        ...
        # Anotaciones de Fondo
        labels_bg = [(15, 85, "Dilema"), (50, 85, "E. Emergente"), (85, 85, "ESTRELLA"),
                     (15, 50, "Cuestionable"), (50, 50, "Core Player"), (85, 50, "High Performer"),
                     (15, 15, "Bajo Rend."), (50, 15, "En Riesgo"), (85, 15, "Eficaz")]
        for x_a, y_a, txt_a in labels_bg:
            fig_9.add_annotation(x=x_a, y=y_a, text=txt_a, showarrow=False, font=dict(color="rgba(0,0,0,0.1)", size=18))

        st.plotly_chart(fig_9, use_container_width=True)

        # Detalle al presionar botón
        if cat_sel:
            st.markdown(f"#### Detalle: {cat_sel}")
            # Lógica de cuadrante para la tabla
            query = ""
            if cat_sel == "ESTRELLA": df_c = df_9[(df_9['Res_Porc'] > 66.6) & (df_9['Comp_Porc'] > 66.6)]
            elif cat_sel == "Dilema": df_c = df_9[(df_9['Res_Porc'] < 33.3) & (df_9['Comp_Porc'] > 66.6)]
            elif cat_sel == "E. Emergente": df_c = df_9[(df_9['Res_Porc'].between(33.3, 66.6)) & (df_9['Comp_Porc'] > 66.6)]
            elif cat_sel == "High Performer": df_c = df_9[(df_9['Res_Porc'] > 66.6) & (df_9['Comp_Porc'].between(33.3, 66.6))]
            elif cat_sel == "Core Player": df_c = df_9[(df_9['Res_Porc'].between(33.3, 66.6)) & (df_9['Comp_Porc'].between(33.3, 66.6))]
            elif cat_sel == "Cuestionable": df_c = df_9[(df_9['Res_Porc'] < 33.3) & (df_9['Comp_Porc'].between(33.3, 66.6))]
            elif cat_sel == "Eficaz": df_c = df_9[(df_9['Res_Porc'] > 66.6) & (df_9['Comp_Porc'] < 33.3)]
            elif cat_sel == "En Riesgo": df_c = df_9[(df_9['Res_Porc'].between(33.3, 66.6)) & (df_9['Comp_Porc'] < 33.3)]
            elif cat_sel == "Bajo Rend.": df_c = df_9[(df_9['Res_Porc'] < 33.3) & (df_9['Comp_Porc'] < 33.3)]
            
            st.dataframe(df_c[['Vendedor', 'Empresa', sel_mes, 'Comp_Porc']].rename(columns={sel_mes: 'Ventas Mes', 'Comp_Porc': '% Competencias'}))

except Exception as e:
    st.error(f"Falla crítica de sistema: {e}")
