...
# Estilos CSS - Blindaje de Interfaz y Fichas
st.markdown("""
    <style>
    ...
    .ficha-card { background-color: #ffffff; border-radius: 10px; padding: 20px; border: 1px solid #e0e0e0; box-shadow: 2px 2px 5px rgba(0,0,0,0.05); text-align: center; }
    </style>
    """, unsafe_allow_html=True)

# --- CARGA Y LIMPIEZA DE DATOS (BLINDAJE TOTAL) ---
...
def load_data():
    ...
    # Índices exactos: Ventas (J, L, N...) y Alcances % (K, M, O...)
...
try:
    ...
    dimension = st.sidebar.radio("NAVEGACIÓN", ["Performance Comercial", "Matriz 9-Box Comercial"])
    ...
    if dimension == "Performance Comercial":
        ...
        f_c1, f_c2, f_c3, f_c4 = st.columns([1, 2, 2, 1.5])
        with f_c1: st.selectbox("AÑO", ["2025"])
        with f_c2: 
        ...
        with f_c4: st.metric("VENDEDORES", len(df_p))

        # Fila 1: Gráficos de Resumen
        ...
        with c2:
            st.markdown("**Top 10 Asesores (Acumulado)**")
        ...
        st.divider()
        # SECCIÓN INDIVIDUAL RESTAURADA
        col_l, col_r = st.columns([1, 2.5])
        with col_l:
            v_sel = st.selectbox("🔎 Buscar Vendedor:", sorted(df_p['Vendedor'].unique()))
        ...
        with col_r:
            ...
            # Gráfico Evolutivo Individual
            fig_evol = go.Figure()
            fig_evol.add_trace(go.Bar(x=lista_meses, y=[v_data[f"{m}_v"] for m in lista_meses], name="Ventas", text_auto='.0f', marker_color='#3498db'))
        ...
        # Gráficos de Pie y Box Plot (Restaurados)
        st.divider()
        g1, g2 = st.columns(2)
        with g1: 
    ...
    # =========================================================
    # DIMENSIÓN: MATRIZ 9-BOX (BOTONES Y FICHA DINÁMICA)
    # =========================================================
    elif dimension == "Matriz 9-Box Comercial":
        st.markdown("### 🎯 Matriz de Talento 9-Box")
        
        m1, m2, m3 = st.columns(3)
        with m1: sel_p = st.selectbox("Periodo de Análisis:", ["Acumulado Anual"] + lista_meses)
        with m2: f_emp9 = st.selectbox("Filtrar Empresa ", ["Todas"] + sorted(df_raw['Empresa'].dropna().unique()))
        with m3: f_loc9 = st.selectbox("Filtrar Localidad ", ["Todas"] + sorted(df_raw['Localidad'].dropna().unique()))

        df_9 = df_raw.copy()
        if f_emp9 != "Todas": df_9 = df_9[df_9['Empresa'] == f_emp9]
        if f_loc9 != "Todas": df_9 = df_9[df_9['Localidad'] == f_loc9]
        
        df_9['X_Axis'] = df_9['Total_%'] if sel_p == "Acumulado Anual" else df_9[f"{sel_p}_%"]

        # 1. BOTONES DE CATEGORÍA
        st.write("**Seleccionar Categoría de Talento:**")
        ...
        for i, b_col in enumerate([bc1, bc2, bc3, bc4, bc5, bc6, bc7, bc8, bc9]):
            if b_col.button(cats[i]): cat_sel = cats[i]

        # 2. GRÁFICO 9-BOX (Iniciales y Esferas)
        fig_9 = px.scatter(
            df_9, x='X_Axis', y='Comp_Total_%', text='Iniciales', color='Empresa',
            size='Size_Marker', hover_name='Vendedor',
            range_x=[-5, 130], range_y=[-5, 110],
            labels={'X_Axis': f'% Alcance ({sel_p})', 'Comp_Total_%': '% Competencias'},
        ...
        # 3. FICHA TÉCNICA DINÁMICA (Sincronizada con Matriz)
        st.divider()
        v_ficha = st.selectbox("🔎 BUSCAR ASESOR PARA VER FICHA TÉCNICA:", ["-- Seleccionar Asesor --"] + sorted(df_9['Vendedor'].unique()))
        
        if v_ficha != "-- Seleccionar Asesor --":
            v_f = df_9[df_9['Vendedor'] == v_ficha].iloc[0]
            
            # Encabezado Ficha (Como imagen 73c0a8)
            k1, k2, k3 = st.columns(3)
            with k1: st.markdown(f"<div class='metric-card'><h2>{v_f['X_Axis']:.1f}%</h2><p>PERFORMANCE ({sel_p})</p></div>", unsafe_allow_html=True)
            with k2: st.markdown(f"<div class='metric-card'><h2>{v_f['Comp_Total_%']:.1f}%</h2><p>COMPETENCIAS</p></div>", unsafe_allow_html=True)
            with k3:
                q = "MIEMBRO CLAVE" if v_f['X_Axis'] > 66 and v_f['Comp_Total_%'] > 66 else "CORE PLAYER"
                st.markdown(f"<div class='metric-card'><h2>{q}</h2><p>ESTADO ACTUAL</p></div>", unsafe_allow_html=True)

            # Detalle Gráfico (Radar de Competencias y Evolución % Alcance)
