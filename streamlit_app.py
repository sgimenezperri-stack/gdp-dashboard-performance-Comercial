...
def load_data():
    ...
    # Asegurar que todas las métricas clave sean numéricas y sin NaNs
...
try:
    ...
    # =========================================================
    # DIMENSIÓN 1: PERFORMANCE COMERCIAL (TOTALMENTE RESTAURADA)
    # =========================================================
    if dimension == "Performance Comercial":
        ...
        with c1:
            ...
            st.plotly_chart(fig_g.update_xaxes(categoryorder='array', categoryarray=lista_meses), use_container_width=True)
        with c2:
        ...
        with col_r:
            ...
            # Gráfico Individual BLINDADO (Eliminado el text_auto conflictivo)
            y_vals = [v_data[f"{m}_v"] for m in lista_meses]
    ...
    # =========================================================
    # DIMENSIÓN 2: MATRIZ 9-BOX (CON CLICKS Y BOTONES)
    # =========================================================
        ...
        # Determinar Eje X
        if sel_p == "Acumulado Anual" or sel_p == "Todos los meses (Promedio)":
            df_9['X_Axis'] = df_9['Alcance_Total_Anual']
        else:
            df_9['X_Axis'] = df_9[f"{sel_p}_%"]
        ...
        # Guardar categoría en session_state para mantenerla
        ...
        for i, b_col in enumerate([bc1, bc2, bc3, bc4, bc5, bc6, bc7, bc8, bc9]):
            if b_col.button(cats[i]): st.session_state.cat_filtrada = cats[i]
        ...
        # 3. INTERACTIVIDAD DE CLICKS
        vendedor_seleccionado = None
        
        if CLICK_HABILITADO:
            st.caption("👈 Haz click en una esfera para ver la ficha del vendedor.")
            # Renderizamos el gráfico usando la librería de eventos
            puntos_click = plotly_events(fig_9, click_event=True, hover_event=False)
            
            if len(puntos_click) > 0:
                # Al hacer click, obtenemos las coordenadas (X, Y) para identificar al vendedor
                click_x = puntos_click[0]['x']
                click_y = puntos_click[0]['y']
                # Buscamos quién tiene esas coordenadas exactas
                match = df_9[(df_9['X_Axis'] == click_x) & (df_9['Comp_Total_%'] == click_y)]
                if not match.empty:
                    vendedor_seleccionado = match.iloc[0]['Vendedor']
        else:
            # Si no instaló la librería, mostramos el gráfico normal
            st.plotly_chart(fig_9, use_container_width=True)
            
        # Buscador manual siempre disponible (como respaldo)
        st.divider()
        ...
        v_ficha = st.selectbox("🔎 BUSCADOR / FICHA TÉCNICA:", opciones_vendedores, index=idx_defecto)

        # 4. MOSTRAR FICHA TÉCNICA
        if v_ficha != "-- Seleccionar Asesor --":
            v_f = df_9[df_9['Vendedor'] == v_ficha].iloc[0]
            
            k1, k2, k3 = st.columns(3)
            with k1: st.markdown(f"<div class='metric-card'><h2>{v_f['X_Axis']:.1f}%</h2><p>PERFORMANCE</p></div>", unsafe_allow_html=True)
            ...
            with gr:
                ...
                st.plotly_chart(fig_l.update_traces(line_color='#2ecc71', line_width=3), use_container_width=True)
